...
class Courier {
      ...
      auth_user.verifyToken(req.token).then((user_details) => {
        if (user_details.verified) {
          courierModel.findOne(
            { $and: [{ user: user_details._id }, { pendingApproval: true }] },
            (err, pending_application) => {
                  ...
                  (err, approved_courier) => {
                    if (approved_courier == null) {
                          ...
                          if (err.name === "MongoError" && err.code === 11000) {
                            res
                              ...
                              .json({
                                success: false,
                                message:
                                  "whatsapp phone number has already been taken",
                              });
                          } else {
                            res
                              .status(203)
                      ...
                      res
                        ...
                        .json({
                          success: false,
                          message: "you are already a verified courier",
                        });
                    }
                  }
                );
          ...
          res
            ...
            .json({
              success: false,
              message: "verify account before applying",
            });
        }
      });
    } catch (e) {
  ...
  setAvailableState(req, res) {
    ...
    try {
      auth_user.verifyToken(req.token).then((user_details) => {
        courierModel.findOne(
          { $and: [{ user: user_details._id }, { verifiedCourier: true }] },
          (err, courier) => {
            if (courier == null) {
              res
                .status(203)
                .json({ success: false, message: "not a verified courier" });
            } else {
              courierModel.findByIdAndUpdate(
                courier._id,
                ...
                (err) => {
                  if (err)
                    res
                      ...
                      .json({
                        success: false,
                        message: "error updating available state",
                        err: err,
                      });
                  res
                    .status(200)
                    .json({
                      success: true,
                      message: "updated availablity state successfully",
                    });
                }
              );
            }
        ...
        .findById(id, (err, courier) => {
          if (err)
            res
              ...
              .json({
                success: false,
                message: "error getting courier details",
              });
        ...
        if (parseInt(data.rating) >= 1 && parseInt(data.rating) <= 5) {
          ...
          ratingModel.create(data, (err, rating) => {
            if (err) {
              res
                .status(203)
                .json({ success: false, message: "error adding rating" });
            } else {
              res
                .status(200)
                .json({ success: true, message: "rating sent successfully" });
              ...
              ratingModel.find({ courier: id }, (err, allRatins) => {
                ...
                var mean = (
                  parseInt(sum) / parseInt(ratings_result.length)
                ).toFixed(2);
          ...
          res
            ...
            .json({
              success: false,
              message: "invalid rating. should be between 1 and 5",
            });
        }
      });
    } catch (e) {
  ...
  getAllcourierStates(req, res) {
    try {
      courierModel.find(
        { $and: [{ verifiedCourier: true }, { suspended: false }] },
        (err, courierList) => {
          ...
          res
            .status(200)
            .json({ success: true, stateList: Array.from(unique_set) });
        }
      );
    } catch (e) {
      res.status(500);
