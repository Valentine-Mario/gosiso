      ...
      auth_user.verifyTokenAdmin(req.token).then((admin) => {
        if (admin == null) {
          res
            ...
            .json({
              success: false,
              message: "unauthorized to access endpoint",
            });
        } else {
          disputeModel.paginate({}, options, (err, dispute_waybills) => {
            if (err)
              res
                ...
                .json({
                  success: false,
                  message: "error getting waybills",
                  err: err,
                });
    ...
    try {
      auth_user.verifyTokenAdmin(req.token).then((admin) => {
        if (admin == null) {
          ...
          disputeModel
            .findById(id, (err, dispute_waybills) => {
              if (err) {
                res
                  .status(203)
                  .json({
                ...
                res
                  .status(200)
                  .json({ success: true, message: dispute_waybills });
              }
            })
            .populate("user")
            .populate("courier")
    ...
    try {
      auth_user.verifyTokenAdmin(req.token).then((admin) => {
        if (admin == null) {
          ...
          waybillModel.paginate(
            { $and: [{ pending: true }, { dispute: false }] },
            options,
            (err, waybills) => {
              if (err)
                res
    ...
    try {
      auth_user.verifyTokenAdmin(req.token).then((admin) => {
        if (admin == null) {
          ...
          waybillModel.paginate(
            { $and: [{ complete: true }, { pending: false }] },
            options,
            (err, waybills) => {
              if (err)
                res
            ...
            .findById(id, (err, waybill_details) => {
              if (err) {
                ...
                waybillModel.findByIdAndUpdate(
                  id,
                  { pending: false, canceled: true, dispute: false },
                  (err) => {
                    if (err) {
                      res
                        .status(203)
                          ...
                          wayBillPayment
                            ...
                            .then((waybill_peyment) => {
                              notificationController.wayBillNotification(
                                ...
                                "Waybill canceled",
                                `Waybill with id ${waybill_details._id} has been canceled by the admin`,
                                null,
                                waybill_details._id
                              );
                              notificationController.wayBillNotification(
          ...
          waybillModel
            .findById(id, (err, waybill_details) => {
              if (err) {
                ...
                waybillModel.findByIdAndUpdate(
                  id,
                  { pending: false, accepted: true, dispute: false },
                  (err) => {
                    if (err) {
                      res
                        .status(203)
                      ...
                      notificationController.wayBillNotification(
                        waybill_details.user,
                        "Waybill accepted",
                        `Waybill with id ${waybill_details._id} has been marked as approved by the admin`,
                        null,
                        waybill_details._id
                      );
                      notificationController.wayBillNotification(
                      ...
                      res
                        ...
                        .json({
                          success: true,
                          message: "way bill marked as accepted successfully",
                        });
                    }
                  }
                );
          ...
          waybillModel
            .findById(id, (err, waybill_details) => {
              if (err) {
                ...
                waybillModel.findByIdAndUpdate(
                  id,
                  { pending: false, complete: true, dispute: false },
                  (err) => {
                    if (err) {
                      res
                        .status(203)
                            ...
                            BalanceController.updateBalace(
                              ...
                              if (success) {
                                ...
                                notificationController.wayBillNotification(
                                  waybill_details.courier.user,
                                  "Waybill Complete",
                                  `Waybill with the id ${waybill_details._id} has been marked as complete by the admin`
                                );
                                notificationController.wayBillNotification(
                                  waybill_details.user,
                                  "Waybill Complete",
