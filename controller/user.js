...
class user {
      ...
      if (data.password.length < 6) {
        ...
        hasher.hash_password(data.password).then((hashed) => {
          ...
          if (
            ...
            data.email.match(email_regex) === null
          ) {
            res
              .status(203)
              .json({
                success: false,
                message:
                  "number must have length of 11 and email must be valid",
              });
          } else {
              ...
              if (err) {
                ...
                balance.createAccount(newUser).then((acc) => {
                  ...
                  auth_user.createToken({ user }).then((login_token) => {
                    res
                      .status(200)
                      .json({ success: true, message: login_token });
                  });
                });
              }
            });
          ...
          userModel.findByIdAndUpdate(user_value._id, data, (err) => {
            if (err)
              res
                ...
                .json({
                  success: false,
                  message: "error approving account, try again",
                });
    ...
    try {
      auth_user.verifyToken(req.token).then((user) => {
        if (!user.verified) {
          ...
          res
            .status(200)
            .json({ success: true, message: "email verification resent" });
        } else {
          res
            .status(203)
        ...
        .then((user) => {
          balance.getBalance(user).then((balance) => {
            courierModel.findOne(
              { $and: [{ verifiedCourier: true }, { user: user._id }] },
              (err, courier) => {
                if (courier == null) {
                  res
                    .status(200)
  ...
  updatePassword(req, res) {
    ...
    try {
      auth_user
        ...
        .then((decoded_user) => {
          hasher
            .compare_password(old_password, decoded_user.password)
            .then((value) => {
              if (!value) {
                res
                  .status(203)
        ...
        .then((decoded_user) => {
          if (data.email.match(email_regex) === null) {
            res
              ...
              .success({
                success: false,
                message: "please pass in a valid email",
              });
          } else {
            hasher
              .compare_password(password, decoded_user.password)
              .then((value) => {
                if (value == true) {
                      ...
                      (err, courierAcc) => {
                        if (err) {
                          res
                            ...
                            .json({
                              success: false,
                              message: "error getting courier account",
                            });
                        } else {
                          if (courierAcc == null) {
                            res
                                ...
                                courierModel.find(
                                  ...
                                  (err, courier) => {
                                    for (var a of courier) {
                                      courierModel.findByIdAndDelete(
                                        a._id,
                                        (err) => {}
                                      );
                                    }
                                ...
                                withdrawModel.find(
                                  ...
                                  (err, withdrawal) => {
                                    for (var a of withdrawal) {
                                      withdrawModel.findByIdAndDelete(
                                        a._id,
                                        (err) => {}
                                      );
                                    }
                                ...
                                waybillModel.find(
                                  ...
                                  (err, waybill) => {
                                    for (var a of waybill) {
                                      waybillModel.findByIdAndDelete(
                                        a._id,
                                        (err) => {}
                                      );
                                    }
                                ...
                                disputeModel.find(
                                  ...
                                  (err, dispute) => {
                                    for (var a of dispute) {
                                      disputeModel.findByIdAndDelete(
                                        a._id,
                                        (err) => {}
                                      );
                                    }
                                ...
                                bank_pending.find(
                                  ...
                                  (err, pending) => {
                                    for (var a of pending) {
                                      bank_pending.findByIdAndDelete(
                                        a._id,
                                        (err) => {}
                                      );
                                    }
                                ...
                                historyModel.find(
                                  ...
                                  (err, history) => {
                                    for (var a of history) {
                                      historyModel.findByIdAndDelete(
                                        a._id,
                                        (err) => {}
                                      );
                                    }
            ...
            userModel.findByIdAndUpdate(user._id, data, (err) => {
              if (err)
                res
                  ...
                  .json({
                    success: false,
                    message: "error updating profile pics",
                  });
              res
                .status(200)
                .json({
        ...
        (err, Current_user) => {
          ...
          auth_user.createTokenResetPassword({ user }).then((token) => {
            WorkQueue.process((job2) => {
              //queue mailing job
              mail
                .forgotPassword(job2.data.email, token)
                .then((email_status) => {
                  console.log(email_status);
                });
              WorkQueue.on("completed", (job2, result) => {
  ...
  resetingPassword(req, res) {
    ...
    try {
      if (data.password !== data.confirm_password || data.password===undefined) {
