...
class Bank{
            ...
            auth_user.verifyTokenAdmin(req.token).then(user=>{
                if(user!==null){
                    bank_pending.paginate({}, options, (err, details)=>{
                        if(err)res.status(203).json({success:false, message:"err retriving details", err:err})
    ...
    getPendingBankById(req, res){
        ...
        try{
            auth_user.verifyTokenAdmin(req.token).then(user=>{
                if(user!==null){
                    bank_pending.findById(id, (err, details)=>{
                        bank_approved.findOne({user:details.user}, (err, current_details)=>{
                            if(err)res.status(203).json({success:false, message:"error getting deatils", err:err})
                            res.status(200).json({success:true, pending_bank:details, current_bank:current_details})
                        })
                    }).populate('user')
                }else{
                    res.status(203).json({success:false, message:"unauthorized to accesss endpoint"}) 
                ...
                if(user!==null){
                    if(data.verify=='true'){
                        bank_pending.findById(id, (err, bank_details)=>{
                            if(err){
                                res.status(203).json({success:false, message:"error getting data", err:err})
                            }else{
                                ...
                                bank_approved.findOneAndUpdate({user:bank_details.user}, new_details, (err)=>{
                                    if(err){
                                        ...
                                        notification.bankChangeNotification(bank_details.user, "Your bank change application has been changed successfully")
                        ...
                        bank_pending.findById(id, (err, bank_details)=>{
                            if(err){
                                ...
                                res.status(200).json({success:true, message:"decline successful"})
                                notification.bankChangeNotification(bank_details.user, "Your bank change application has been declined")
                                bank_pending.findByIdAndDelete(id, (err)=>{
                                    //do nothing
                                })
                            }
