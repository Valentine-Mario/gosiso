  ...
  adminLogin(req, res) {
    ...
    try {
      adminModel.findOne({ email: data.email }, (err, user) => {
        if (user !== null) {
          hasher
            .compare_password(data.password, user.password)
            .then((value) => {
              if (value) {
                user = user._id;
    ...
    try {
      auth_user.verifyTokenAdmin(req.token).then((admin) => {
        if (admin == null) {
          ...
          courierModel.paginate(
            {
              $and: [
                { pendingApproval: data.pendingApproval },
                { verifiedCourier: data.verifiedCourier },
              ],
            },
            options,
            (err, couriers) => {
            ...
            .findById(id, (err, courier) => {
              courierModel.findByIdAndUpdate(id, data, (err) => {
                if (err) {
                  ...
                  res
                    .status(200)
                    .json({ success: true, message: "declined successful" });
              ...
              for (var i = 0; i < data.wareHouseImage.length; i++) {
                cloud.pics_upload(data.wareHouseImage[i].path).then((val) => {
                  ...
                  if (count == img.length) {
                    ...
                    cloud
                      .pics_upload(data.locationImage)
                      .then((location_img) => {
                        ...
                        courierModel.findByIdAndUpdate(id, data, (err) => {
                          if (err) {
                            ...
                            userModel.findByIdAndUpdate(
                              ...
                              { verifiedCourier: true },
                              (err) => {
                                notificationModel.approveNotification(
                                  courier.user
                                );
            ...
            courierModel.findByIdAndUpdate(id, { suspended: true }, (err) => {
              if (err) {
                res
                  ...
                  .json({
                    success: false,
                    message: "error updating courier status",
                  });
              } else {
                res
                  .status(200)
                ...
                notificationModel.wayBillNotification(
                  courier_details.user,
                  "suspension notice",
                  "You have been temporarily suspended as a courier"
                );
              }
            });
          });
            ...
            courierModel.findByIdAndUpdate(id, { suspended: false }, (err) => {
              if (err) {
                ...
                notificationModel.wayBillNotification(
                  courier_details.user,
                  "unsuspension notice",
                  "You have been unsuspended. You can now be booked again"
                );
              }
            });
          });
  ...
  getSuspendedOrUnsuspended(req, res) {
    ...
    auth_user.verifyTokenAdmin(req.token).then((admin) => {
      if (admin == null) {
        res
          .status(203)
          .json({ success: false, message: "unauthorized to access endpoint" });
      } else {
        courierModel.paginate(
          ...
          (err, couriers) => {
            if (err)
              res
                .status(203)
                .json({ success: false, message: "error getting couriers" });
            res.status(200).json({ success: true, message: couriers });
          }
        );
      }
        ...
        if (admin == null) {
          ...
          courierModel.findById(id, (err, courier_details) => {
            if (courier_details.wareHouseImage.includes(data.wareHouseImage)) {
              courier_details.wareHouseImage.splice(
                courier_details.wareHouseImage.indexOf(data.wareHouseImage),
                1
              );
      ...
      auth_user.verifyTokenAdmin(req.token).then((admin) => {
        if (admin == null) {
          ...
          courierModel.paginate(
            {
              $or: [
                { state: { $regex: value, $options: "gi" } },
                { city: { $regex: value, $options: "gi" } },
              ],
            },
            options,
            (err, couriers) => {
              if (err)
                res
                  ...
                  .json({
                    success: false,
                    message: "error searching courier",
                    err: err,
                  });
              res.status(200).json({ success: true, message: couriers });
            }
      ...
      auth_user.verifyTokenAdmin(req.token).then((admin) => {
        if (admin == null) {
          ...
          userModel.paginate(
            {
              $or: [
                { firstName: { $regex: value, $options: "gi" } },
                { lastName: { $regex: value, $options: "gi" } },
              ],
            },
            options,
            (err, users) => {
